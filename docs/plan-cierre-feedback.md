...
| FB-06 | Docker local incompleto | Se actualizo y valido Docker Compose como demo minima con Eureka | `docker-compose.yml`, `discovery-server/Dockerfile`, README | Demo incluye discovery-server, Gateway, pedido, cliente, producto y estado; endpoints principales respondieron 200 | Corregido para demo minima |
| FB-07 | Render pendiente | Se documento Render y se desplego `discovery-server` como primer servicio | `docs/render-deploy.md` | URL real `https://discovery-server-gjd0.onrender.com`; Gateway y clientes pendientes | Parcial |
...
|---|---|---|
| Docker Compose demo minima | Demo minima validada del flujo principal con Eureka; no levanta los 10 microservicios | Si la pauta exige todos los servicios en Docker, debe ampliarse |
| Render parcial | `discovery-server` ya esta desplegado; faltan Gateway y microservicios con URLs reales | Debe completarse antes de entregar por AVA si es obligatorio |
...
## Evidencia Docker Compose Eureka

La demo minima validada incluye `discovery-server`, `api-gateway`, `cliente-service`, `producto-service`, `pedido-service` y `estado-service`.

| Validacion | Resultado |
|---|---|
| `docker compose config` | Correcto |
| `docker compose build` | Correcto |
| `docker compose up -d` y `docker compose ps` | 6 contenedores `Up` |
| `curl.exe http://localhost:8761` | HTTP 200 |
| `curl.exe http://localhost:8080/actuator/health` | HTTP 200 |
| `curl.exe http://localhost:8080/api/clientes` | HTTP 200 |
| `curl.exe http://localhost:8080/api/productos` | HTTP 200 |
| `curl.exe http://localhost:8080/api/pedidos` | HTTP 200 |
| Eureka registry | `API-GATEWAY`, `CLIENTE-SERVICE`, `PRODUCTO-SERVICE`, `PEDIDO-SERVICE`, `ESTADO-SERVICE` en `UP` |

## Evidencia Render Discovery Server

| Evidencia | Resultado |
|---|---|
| Render deploy `discovery-server` | Exitoso |
